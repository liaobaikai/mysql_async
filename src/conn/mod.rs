    ...
    async fn switch_to_ssl_if_needed(&mut self) -> Result<()> {
            ...
            .contains(CapabilityFlags::CLIENT_SSL)
        {
                ...
                .contains(CapabilityFlags::CLIENT_SSL)
            {
                ...
                // The server `skip_ssl` is `1`, need get public key from server
                return Ok(());
            }
        ...
        match packet.first() {
            ...
            Some(0x01) => match packet.get(1) {
                ...
                Some(0x04) => {
                    ...
                    // Updateby: liaobaikai
                    // updateTime: 2025-11-02
                    // The server `skip_ssl` is `0`
                    // line 504: "self.inner.capabilities = handshake.capabilities() & self.inner.opts.get_capabilities();"
                    // 
                    if (self.capabilities().contains(CapabilityFlags::CLIENT_SSL) && self.is_secure()) || self.is_socket() {
                        self.write_packet(pass).await?;
                    } else {
                        // input public_key from user
                        self.inner.server_key = self.opts().public_key().clone();

                        if self.inner.server_key.is_none() && self.opts().get_server_public_key() {
